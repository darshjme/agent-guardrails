...
<img src="assets/agent-guardrails-hero.png" alt="agent-guardrails — Vedic Arsenal by Darshankumar Joshi" width="100%" />
...
[![Zero Dependencies](https://img.shields.io/badge/Dependencies-Zero-brightgreen?style=flat-square)](https://github.com/darshjme/agent-guardrails)
[![Tests](https://img.shields.io/badge/Tests-Passing-success?style=flat-square)](https://github.com/darshjme/agent-guardrails/actions)
...
`agent-guardrails` implements **production guardrails for llm agent outputs — schema validation, content safety, retry logic**
...
```mermaid
flowchart LR
    A[Input] --> B[agent-guardrails]
    B --> C{Process}
    C -- Success --> D[Output]
    C -- Error --> E[Handle / Retry]
    E --> B
    style B fill:#6b21a8,color:#fff
    note["Guardrails — Zero Dependencies"]
```
...
Production LLM systems fail in predictable ways. `agent-guardrails` solves the **guardrails** failure mode with:
...
`agent-guardrails` is part of **[darshjme/arsenal](https://github.com/darshjme/arsenal)** — a collection of 100 focused Python libraries for LLM agent infrastructure.

Each library solves exactly one problem. Together they form a complete stack.

```
pip install agent-guardrails  # this library
